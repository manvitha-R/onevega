...
import { FaPlay, FaPen, FaTrash, FaTasks, FaDatabase, FaTable, FaBook, FaCogs, FaClock } from "react-icons/fa";
...
export default function Page() {
  ...
  const [loadingPromptsRepository, setLoadingPromptsRepository] = useState(false);
 ...
 useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(
        "http://143.110.180.27:8002/main-boards/boards/ai-documentation/",
        {
          headers: {
      ...
      const filteredData = response.data.filter(
        (item: { board_id: any }) => String(item.board_id) === String(boardId)
      );
  ...
  try {
    ...
    const response = await fetch(
      `http://143.110.180.27:8002/main-boards/boards/ai-documentation/${id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `http://143.110.180.27:8002/main-boards/boards/data-management-table/get_all_tables_with_files`,
          {
            headers: {
              "X-API-Key": "xxAJf365FZZidPt496lk9M2XDbvQCMKevOSuBgx2k6BAjp3ALe4vLTjXtcmgatoQtvsSLED3lx7zEgyHcohd1Wa2iJWTlukzQTuauvTbGYjSgMtFq5AUQLuAcMW44mp",
            },
  ...
  useEffect(() => {
    const fetchPrompts = async () => {
      ...
      try {
        const response = await fetch(
          `http://143.110.180.27:8002/main-boards/boards/prompts/boards/${boardId}`,
          {
            headers: {
  ...
  const handleRunnPrompt = async (promptText: string) => {
    ...
    try {
      ...
      const url = new URL(
        "http://143.110.180.27:8002/main-boards/boards/prompts/run_prompt_v2/"
      );
...
  };
  ...
  const handleRePrompt = async () => {
    try {
      ...
      const response = await axios.post(
        'http://143.110.180.27:8002/main-boards/boards/prompts/re_prompt',
  ...
  const handleDeletes = async (row: TableRow) => {
    ...
    try {
      const response = await fetch(
        `http://143.110.180.27:8002/main-boards/boards/data-management-table/${row.id}`,
        {
          method: "DELETE",
...
  };
  ...
  const handleDeletePrompt = async (promptId: string) => {
    try {
      const response = await fetch(
        `http://143.110.180.27:8002/main-boards/boards/prompts/${promptId}`,
        {
          method: "DELETE",
          headers: {
            "X-API-Key": "xxAJf365FZZidPt496lk9M2XDbvQCMKevOSuBgx2k6BAjp3ALe4vLTjXtcmgatoQtvsSLED3lx7zEgyHcohd1Wa2iJWTlukzQTuauvTbGYjSgMtFq5AUQLuAcMW44mp",
  ...
  const handleSavePrompt = async () => {
    ...
    const loggedInUserName = localStorage.getItem('loggedInUserName') || "Unknown User"; // Fallback to "Unknown User" if not found
    ...
    const url = editPromptId
      ? `http://143.110.180.27:8002/main-boards/boards/prompts/${editPromptId}` // Update endpoint
      : `http://143.110.180.27:8002/main-boards/boards/prompts/`; // Create endpoint
  ...
  const handleEditClick = (id: string | null, item: { id?: string; name?: string; configuration_details: any; }) => {
    setDocId(id); // or setDocId(item.id) based on your structure
    setEditRowId(id);
    setEditValues(item.configuration_details); // assuming this holds the config details you want to edit
  };
 ...
 const handleSaveClick = async (id: string) => {
  ...
  const handleSubmitfile = async (e: { preventDefault: () => void }) => {
    ...
    try {
      const response = await fetch(
        `http://143.110.180.27:8002/main-boards/boards/data-management-table/status/upload/${selectedTableId}`, // Ensure you're using selectedTableId
        {
          method: "POST",
          headers: {
            "X-API-Key": "xxAJf365FZZidPt496lk9M2XDbvQCMKevOSuBgx2k6BAjp3ALe4vLTjXtcmgatoQtvsSLED3lx7zEgyHcohd1Wa2iJWTlukzQTuauvTbGYjSgMtFq5AUQLuAcMW44mp",
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      const response = editRow
        ? await fetch(
          `http://143.110.180.27:8002/main-boards/boards/data-management-table/${editRow.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        : await fetch(
          `http://143.110.180.27:8002/main-boards/boards/data-management-table/create`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div className="p-10 h-screen overflow-y-auto">
      ...
      <div className="flex space-x-24 border-b border-gray-400 mb-10">
        <button
          className={`py-2 px-4 ${activeTab === "prompts" ? "border-b-4 border-emerald-600" : ""}`}
        ...
        <button
          className={`py-2 px-4 ${activeTab === "repository" ? "border-b-4 border-emerald-600" : ""}`}
          onClick={() => setActiveTab("repository")}
        >
          <b>Prompts Repository</b>
        </button>
        <button
          className={`py-2 px-4 ${activeTab === "tables" ? "border-b-4 border-emerald-600" : ""}`}
          onClick={() => setActiveTab("tables")}
        >
          <b>Manage Tables</b>
        </button>
        <button
          className={`py-2 px-4 ${activeTab === "documentation" ? "border-b-4 border-emerald-600" : ""}`}
          onClick={() => setActiveTab("documentation")}
        >
          <b>AI Documentation</b>
        </button>
        <button
          className={`py-2 px-4 ${activeTab === "master" ? "border-b-4 border-emerald-600" : ""}`}
          onClick={() => setActiveTab("master")}
        >
          <b>Master Settings</b>
        </button>
        <button
          className={`py-2 px-4 ${activeTab === "timeline" ? "border-b-4 border-emerald-600" : ""}`}
      ...
      {activeTab === "prompts" && (
        <>
          ...
          <div className="flex justify-end">
            <button
              className="new-prompt-btn py-2 px-4 bg-emerald-500 text-white rounded-md"
          ...
          {prompts.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 p-6">
              {prompts.map((prompt, index) => (
                <div
                  key={prompt.id}
                  className="prompt-card border rounded-lg shadow-md p-4 bg-white transition-all duration-300 ease-in-out transform hover:scale-110 hover:shadow-2xl"
                  style={{ minWidth: "200px" }}
                >
                  <p
                    className="text-lg font-semibold line-clamp-6 overflow-hidden text-ellipsis mb-6"
                  ...
                  <div className="prompt-footer">
                    <div className="mb-2">
                      <p className="text-sm text-gray-600">Created By: {prompt.user_name || "Unknown User"}</p>
                ...
                prompts.map((prompt, index) => (
                  ...
                  >
                    ...
                    <div className="mt-4 text-sm">
                      <p className="opacity-90 ">Created By: {prompt.user_name}</p>
      ...
      {isResultModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-full relative w-[99%] h-[690px] overflow-y-auto">
                        ...
                        <div className="my-4 flex flex-wrap justify-center gap-6">
                          ...
                          <div className="w-full max-w-[500px] flex-1">
                            ...
                            {getChartData("bar") ? (
                              <div style={{ height: "400px" }}> {/* Fixed Height */}
                                <Bar
                                  data={getChartData("bar") as ChartData<"bar", number[], string>}
                                  options={{
                                    maintainAspectRatio: false, // Ensures it respects the height
                          ...
                          <div className="w-full max-w-[500px] flex-1">
                            ...
                            {getChartData("line") ? (
                              <div style={{ height: "400px" }}> {/* Fixed Height */}
                                <Line
                                  data={getChartData("line") as ChartData<"line", number[], string>}
                                  options={{
                                    maintainAspectRatio: false, // Ensures it respects the height
                                    plugins: { legend: { display: true, position: "top" } },
                                    scales: { y: { beginAtZero: true } },
                                      ...
                                      <tr key={index} className="border-b border-gray-200">
                                        ...
                                        <td className="px-4 py-3 text-center">
                                          {editRowId === item.id ? (
                                            ...
                                            >
                                              <FiSave className="mr-1" size={16} style={{ color: "green" }} />
                                            </button>
                                          ) : (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg  w-[99%] h-[690px] max-h-[90vh] overflow-y-auto relative">
            <button
              onClick={handleCloseModal}
